...
public class ProductController {
    ...
    public List<CartItem> cartItems(){
        return cartService.getAllProducts()
                .stream()
                .map(p->new CartItem(p.getId(),p.getName(),p.getPrice(),
                        p.getDescription(),p.getQuantity(),p.getLastUpdate()))
    ...
    public String checkoutTransfer(@ModelAttribute("total") double total, RedirectAttributes redirectAttributes){
        ...
        redirectAttributes.addFlashAttribute(" not transfer-error",true);
        return "redirect:/ui/checkout-view";
    }
    ...
    public String toCheckoutView(Model model){
        model.addAttribute("transfer-error", model.containsAttribute("transfererror"));
        return "checkoutView";
    }
    ...
    public double totalAmount(){
        return cartService.getAllProducts()
                .stream().map(p->p.getQuantity() * p.getPrice())
                .mapToDouble(i -> i)
                .sum();
    }
